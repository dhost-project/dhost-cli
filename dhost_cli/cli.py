...
def main():
    ...
    parser.add_argument(
        "-p", "--password", help="Connect to API with username and password."
    )
    ...
    parser.add_argument(
        "--disable-color", action="store_true", help="Disable colored output."
    )
    ...
    delete_ipfs_dapp = ipfs_dapp_sub.add_parser(
        "delete", help="Delete an IPFS dapp."
    )
    delete_ipfs_dapp.add_argument("ipfs_dapp_slug")
    deploy_ipfs_dapp = ipfs_dapp_sub.add_parser(
        "deploy", help="Deploy an IPFS dapp."
    )
